...
### [MANDATORY] 코드 변경 전 — Open PR 충돌 점검

서비스(oms-core, oms-plan) 코드를 수정하기 전에 반드시 수행한다:

1. `gh pr list --state open`으로 해당 서비스의 열린 PR을 확인한다
2. 열린 PR이 있으면 `gh pr diff {number} --name-only`로 PR이 수정한 파일 목록을 확인한다
3. 내가 수정할 파일과 겹치면 사용자에게 **충돌 가능성을 경고**하고 진행 여부 승인을 받는다
4. 승인 없이 겹치는 파일을 수정하지 않는다
