...
- ❌ **이전 메시지의 요청을 이후 메시지에 자동 적용 금지** — 각 메시지는 독립적이다. 이전에 "PR 올려", "push해" 등을 말했더라도, 현재 메시지에 해당 키워드가 없으면 수행하지 마라
- ❌ **문제 지적을 작업 지시로 해석 금지** — "이거 잘못됐는데?", "이건 안 되지!" 등은 문제를 알려주는 것이지, 수정+커밋+push를 한꺼번에 지시하는 것이 아니다. 파일 수정만 하고, git 작업은 별도 키워드를 기다려라
...
> git 트리거 키워드와 상세 규칙은 `/develop` 스킬 GIT_RULES 참조
