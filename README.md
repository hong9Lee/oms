...
│   │   ├── auto-spotless.sh   # Java 수정 후 자동 포맷팅
...
| 항목 | 내용 |
|------|------|
| **시점** | PostToolUse (Edit, Write) |
| **목적** | Java 파일 수정 후 자동 포맷팅 |

Agent가 `.java` 파일을 수정하면 `./gradlew spotlessApply -q`를 자동 실행한다. 비-Java 파일(`.md`, `.json` 등)은 무시한다. 실패해도 작업을 중단하지 않는다 (`|| true`).
...
```json
{
  "hooks": {
    ...
    "PostToolUse": [
      { "matcher": "Edit|Write", "hooks": [{ "command": ".claude/hooks/auto-spotless.sh" }] }
    ],
    "PreCompact": [
